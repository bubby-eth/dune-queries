...
-- collateral (0-1 fraction), and first/latest staking activity. Labels come
-- from the shared curated map with cex.addresses fallback.
...
SELECT
  ...
  COALESCE(lm.label, cex.distinct_name, '') AS label,
...
FROM per_account p
LEFT JOIN label_map lm ON lm.addr = p.account
LEFT JOIN cex.addresses AS cex
  ON cex.address = p.account AND cex.blockchain = 'ethereum'
