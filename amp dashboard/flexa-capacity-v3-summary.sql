...
-- total_net_tokens = sum of reservation.tokenAmount over every pool.
...
-- 6) final aggregation: total reserved AMP x USD price
SELECT
  SUM(CASE WHEN t.reservation_id IS NULL THEN d.delta ELSE CAST(0 AS int256) END) / 1e18 AS total_net_tokens,
  SUM(CASE WHEN t.reservation_id IS NULL THEN d.delta ELSE CAST(0 AS int256) END) / 1e18
    * MAX(pq.price)                                                                      AS total_usd_value
FROM deltas d
