-- Current reserved collateral for every Flexa Capacity v3 pool.
-- Pool list from https://api.flexa.co/collateral_pools (address = id, name = entity.name).
--
...
WITH
  ...
  -- every vault reservation belonging to a pool
  reservations AS (
...
SELECT
  ...
  100.0 * r.reserved_raw / SUM(r.reserved_raw) OVER ()         AS pct_of_total,
  r.last_change
FROM reserved_by_pool r
CROSS JOIN amp_price p
ORDER BY reserved_amp DESC;
