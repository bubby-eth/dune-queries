  ...
  capacity_query AS (
    SELECT
      ...
      CONCAT(
        CAST(
          COALESCE(
            CAST(
              COALESCE(
                TRY_CAST('<a href="https://etherscan.io/tx/' AS VARCHAR),
                ''
              ) AS VARCHAR
            ),
            ''
        ...
        CAST(
          COALESCE(
            CAST(
              COALESCE(
                TRY_CAST(
                  REPLACE(TRY_CAST(tr."evt_tx_hash" AS VARCHAR), '\', '0') AS VARCHAR
                ),
                ''
              ) AS VARCHAR
            ),
        ...
        CAST(
          COALESCE(
            CAST(
              COALESCE(TRY_CAST('Etherscan' AS VARCHAR), '') AS VARCHAR
            ),
            ''
          ) AS VARCHAR
        ),
      ...
      CASE
        ...
        WHEN TRY_CAST(tx."data" AS VARCHAR) LIKE '%ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffcccccccc186b8f18e6ee4b42706d7f8b3445d8dfc790c524e3990ef014e7c578%' THEN 'Litcoin'
    ...
    WHERE
      ...
      AND tr."evt_block_time" >= CURRENT_TIMESTAMP - INTERVAL '30' day
    UNION ALL
    SELECT
      tr."evt_block_time",
      - (CAST(tr."value" AS INT256) / 1e18) AS balance,
      ...
      CASE
        ...
        WHEN TRY_CAST(tx."data" AS VARCHAR) LIKE '%x2036a94dcccccccc186b8f18e6ee4b42706d7f8b3445d8dfc790c524e3990ef014e7c578%' THEN 'Litcoin'
        WHEN TRY_CAST(tx."data" AS VARCHAR) LIKE '%2036a94dcccccccc186b8f18e6ee4b42706d7f8b3445d8dfc790c524e3990ef014e7c578000000000000000000000000706d7f8b3445d8dfc790c524e3990ef014e7c578000000000000000000000000%' THEN 'Litcoin'
